...
corridas<-1
for (n in c(40, 50, 60, 100, 150)) {
  for(k in seq(10, 50, 10)){
...
suppressMessages(library(doParallel))
registerDoParallel(makeCluster(detectCores() - 1))
largos <- foreach(r = 1:corridas, .combine=c) %dopar% propaga(r)
stopImplicitCluster()
largos<-matrix(largos, byrow=TRUE)
largos<-cbind(largos,n,k)
...
#resultados$Semillas<-as.factor(resultados$Semillas)
#png("Dimension.png")
#ggplot(data=resultados,aes(x=Semillas, y=Longitud, fill=Dimension)
#)
#geom_boxplot(position=position_doge(1))+
 # facet_grid(.~Dimension)+
  #scale_y_continuous(name="Longitud")+
  #scale_x_discrete(name="Semillas")
dev.off()