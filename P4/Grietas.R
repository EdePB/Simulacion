...
inicio <- function() {
  ...
  xg <- NULL
  yg <- NULL # direccion en que va mi cabeza de la grieta
  ...
  return(c(xg, yg))#direccion a donde se va a propagar a grieta
}
...
for (dx in -1:1) {
  for (dy in -1:1) {
    if (dx != 0 | dy != 0) { # descartar la posicion misma
      vp <- rbind(vp, c(dx, dy))#declara la vecindad
    }
  }
}
names(vp) <- c("dx", "dy")
vc <- dim(vp)[1]#dimension de ocho vecinos
...
propaga <- function(replica) {
  ...
  yg <- i[2]
  largo <- 0
  while (TRUE) { # hasta que la propagacion termine se rompe solo cuando no pueda avanzar
    grieta[yg, xg] <- 0 # usamos el cero para marcar la grieta
    largo <-  largo + 1
    frontera <- numeric()#aqui se guardan los datos de los ocho vecinos
    interior <- numeric()
    for (v in 1:vc) {
      vecino <- vp[v,]
      xs <- xg + vecino$dx # columna del vecino potencial #$elige que columna o fila
      ys <- yg + vecino$dy # fila del vecino potencial
      if (xs > 0 & xs <= n & ys > 0 & ys <= n) { # no sale de la zona que quede dentro de la matriz
        if (grieta[ys, xs] > 0) { # aun no hay grieta ahi 
          if (voronoi[yg, xg] == voronoi[ys, xs]) {
            interior <- c(interior, v)#determina si estas en el interior de una zona del mismo color
          } else { # frontera
            frontera <- c(frontera, v)#determina si estas en frontera con otro color
          }
        }
      }
    }
    ...
    if (elegido > 0) { # si se va a propagar
      vecino <- vp[elegido,]#se pide todos los valores en elegido
      xg <- xg + vecino$dx
      yg <- yg + vecino$dy
    } else {
